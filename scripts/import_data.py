...
def import_admission_scores(conn, valid_codes: set):
    # 遍历 data 下除“院校数据”外的子目录，导入所有 *.csv
    total = 0
    for child in REGION_DATA_DIR.iterdir():
        if not child.is_dir():
            continue
        if child.name == "院校数据":
            continue
        ...
        for csv in child.glob("*.csv"):
            try:
                ...
                # 丢弃关键字段为空的行
                df = df.dropna(subset=["ADMISSION_YEAR", "MIN_SCORE", "MIN_RANK"]).copy()
